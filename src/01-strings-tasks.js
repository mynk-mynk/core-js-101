...
function encodeToRot13(str) {
  const lettersCap = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const lettersLow = 'abcdefghijklmnopqrstuvwxyz';
  let encodedStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (lettersCap.includes(str[i])) {
      encodedStr += lettersCap[(lettersCap.indexOf(str[i]) + 13) % 26];
    } else if (lettersLow.includes(str[i])) {
      encodedStr += lettersLow[(lettersLow.indexOf(str[i]) + 13) % 26];
    } else {
      encodedStr += str[i];
    }
  }
